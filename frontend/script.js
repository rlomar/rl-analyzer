...
function renderTeamAnalysis(t){const s=document.getElementById("team-section");s.classList.remove("hidden");const modeLabels={"1v1":"فردي 1v1","2v2":"زوجي 2v2","3v3":"فريق 3v3","scrim":"سكريم"};const ml=modeLabels[gameMode]||gameMode;let h=`<div class="card"><h2>🏆 تحليل الفريق — ${ml}</h2><div class="team-grid">`;for(const k of["blue","orange"]){const v=t[k];if(!v)continue;const wn=v.goals>v.opponent_goals,dr=v.goals===v.opponent_goals;const teamColor=k==="blue"?"#4a9eff":"#ff8c33";h+=`<div class="team-card ${wn?"team-won":dr?"team-drawn":"team-lost"}" style="border-top:3px solid ${teamColor}"><div class="team-header"><span class="team-name" style="color:${teamColor}">${v.name}</span><span class="team-result">${v.goals} - ${v.opponent_goals} ${wn?"✅":dr?"⚖️":"❌"}</span></div><div class="team-stats"><div class="team-stat"><span class="label">🚀 سرعة الفريق</span><span class="value">${v.avg_speed}</span></div><div class="team-stat"><span class="label">⛽ boost</span><span class="value">${v.avg_boost}</span></div><div class="team-stat"><span class="label">🎯 تسديدات</span><span class="value">${v.total_shots}</span></div><div class="team-stat"><span class="label">🛑 تصديات</span><span class="value">${v.total_saves}</span></div><div class="team-stat"><span class="label">🎯 تمريرات</span><span class="value">${v.total_assists}</span></div><div class="team-stat"><span class="label">💥 ديمو</span><span class="value">${v.demos_inflicted}</span></div><div class="team-stat"><span class="label">📍 مسافة للكرة</span><span class="value">${v.avg_distance_ball}</span></div><div class="team-stat"><span class="label">👥 تماسك الفريق</span><span class="value">${v.avg_distance_mates}</span></div><div class="team-stat"><span class="label">⭐ مجموع سكور</span><span class="value">${v.total_score}</span></div></div><div class="tips-section"><h3>💡 نصائح الفريق</h3>${v.tips.map(t=>`<div class="tip-card priority-${t.priority}"><div class="tip-header"><span class="tip-title">${t.title}</span><span class="tip-priority ${t.priority}">${t.priority==="high"?"🚨 مهم":t.priority==="medium"?"⚡ متوسط":"✅ ممتاز"}</span></div><p class="tip-advice">${t.advice}</p></div>`).join("")}</div></div>`;)||(h+=`<p style="color:#8892b0;">لا توجد بيانات كافية</p>`);s.innerHTML=h;}
