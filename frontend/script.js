...
function loadMatchesTab(){
    ...
    fetch("/api/user/profile").then(r=>r.json()).then(d=>{
        ...
        let html='<div class="table-wrap"><table class="history-table"><thead><tr><th>#</th><th>الطور</th><th>الخريطة</th><th>أ/ت/تص</th><th>دقة</th><th>سكور</th><th>نتيجة</th><th>التاريخ</th><th></th></tr></thead><tbody>';
        recent.forEach((g,i)=>{
            ...
            html+=`<tr><td>#${recent.length-i}</td><td>${g.game_mode||"-"}</td><td>${g.map_name||"-"}</td><td>${g.goals||0}/${g.assists||0}/${g.saves||0}</td><td>${Math.round(g.shooting_pct||0)}%</td><td>${g.score||0}</td><td style="color:${w?"#00c853":dr?"#ffab00":"#ff1744"}">${w?"فوز":dr?"تعادل":"خسارة"}</td><td style="font-size:12px;color:#5a6a8a">${g.uploaded_at?g.uploaded_at.slice(0,10):"-"}</td>
            <td>${g.replay_id?`<a href="/api/replay/${g.replay_id}/download" class="btn btn-sm" style="padding:2px 8px;font-size:11px;text-decoration:none;">⬇</a>`:""}</td></tr>`;
        });
...
function showResults(data, selectedPlayer) {
    ...
    sorted.forEach((p,i)=>{
        ...
        html+=`<div class="player-card team-${tc}" style="${isMe?'outline:2px solid #5b8cff;outline-offset:-2px;':''}"><div class="player-header"><span class="player-name">${isMe?'👤 ':''}${p.name}</span><span class="team-badge ${tc}">${tc==="blue"?"الأزرق":"البرتقالي"}${isMe?' ⭐':''}</span></div>
...
function renderTeamAnalysis(t) {
    const s = document.getElementById("team-section");
    s.classList.remove("hidden");
    const modeLabels = {"1v1": "فردي 1v1", "2v2": "زوجي 2v2", "3v3": "فريق 3v3", "scrim": "سكريم"};
    const ml = modeLabels[gameMode] || gameMode;
    let h = `<div class="card"><h2>🏆 تحليل الفريق — ${ml}</h2><div class="team-grid">`;
    for (const k of ["blue", "orange"]) {
        const v = t[k];
        if (!v) continue;
        const wn = v.goals > v.opponent_goals, dr = v.goals === v.opponent_goals;
        const teamColor = k === "blue" ? "#4a9eff" : "#ff8c33";
        let card = `<div class="team-card ${wn ? "team-won" : dr ? "team-drawn" : "team-lost"}" style="border-top: 3px solid ${teamColor}">
            <div class="team-header"><span class="team-name" style="color: ${teamColor}">${v.name}</span>
            <span class="team-result">${v.goals} - ${v.opponent_goals} ${wn ? "✅" : dr ? "⚖️" : "❌"}</span></div>
            <div class="team-stats">
                <div class="team-stat"><span class="label">🚀 سرعة الفريق</span><span class="value">${v.avg_speed}</span></div>
                <div class="team-stat"><span class="label">⛽ boost</span><span class="value">${v.avg_boost}</span></div>
                <div class="team-stat"><span class="label">🎯 تسديدات</span><span class="value">${v.total_shots}</span></div>
                <div class="team-stat"><span class="label">🛑 تصديات</span><span class="value">${v.total_saves}</span></div>
                <div class="team-stat"><span class="label">🎯 تمريرات</span><span class="value">${v.total_assists}</span></div>
                <div class="team-stat"><span class="label">💥 ديمو</span><span class="value">${v.demos_inflicted}</span></div>
                <div class="team-stat"><span class="label">📍 مسافة للكرة</span><span class="value">${v.avg_distance_ball}</span></div>
                <div class="team-stat"><span class="label">👥 تماسك الفريق</span><span class="value">${v.avg_distance_mates}</span></div>
                <div class="team-stat"><span class="label">⭐ مجموع سكور</span><span class="value">${v.total_score}</span></div>
            </div>
            <div class="tips-section"><h3>💡 نصائح الفريق</h3>`;
        if (v.tips && v.tips.length) {
            card += v.tips.map(t => `<div class="tip-card priority-${t.priority}">
                <div class="tip-header"><span class="tip-title">${t.title}</span>
                <span class="tip-priority ${t.priority}">${t.priority === "high" ? "🚨 مهم" : t.priority === "medium" ? "⚡ متوسط" : "✅ ممتاز"}</span></div>
                <p class="tip-advice">${t.advice}</p></div>`).join("");
        } else {
            card += `<p style="color: #8892b0;">لا توجد بيانات كافية</p>`;
        }
        card += `</div></div>`;
        h += card;
        s.innerHTML = h;
...
function handleSearch(){
    ...
    searchTimeout=setTimeout(()=>{
        fetch(`/api/players/search?q=${encodeURIComponent(inp)}`).then(r=>r.json()).then(d=>{
            ...
            const rankIcon={Bronze:"🟤",Silver:"⚪",Gold:"🟡",Plat:"🟢",Diamond:"🔵",Champ:"🟣",GC:"🔴",SSL:"💜"};
            res.innerHTML=pl.map(p=>{
                const isUser=p.source==="user";
                const tag=p.hash_tag?`<span class="search-result-tag">#${p.hash_tag}</span>`:"";
                const displayName=isUser?`${p.username||p.player_name}${tag}`:p.player_name;
                const rank=playerRank(p.total_games||0);
                const rankDot=rankIcon[rank]||"⚪";
                const statsTxt=isUser?`<span class="search-result-user">👤 مستخدم · ${rankDot} ${rank}</span>`:`${rankDot} ${rank} · ${p.total_games} مباريات · ${p.total_goals} هدف`;
                const onClick=isUser?`showPlayerProfile('${p.username||p.player_name}')`:`showPlayerProfile('${p.player_name}')`;
                return `<div class="search-result-item search-result-${p.source}" onclick="${onClick}">
                    <span class="search-result-name">${isUser?"👤 ":""}${displayName}</span>
                    <span class="search-result-stats">${statsTxt}</span>
                </div>`;
            }).join("");
